...
The Hermes 0.20.0 promotion train updates `build/agentera-runtime-seed.lock.json` only in the release branch: it binds `runtime-v0.20.0-agentera.1-rc.1`, source `ae746df6556f1d496f9dd49c850cc6133997e317`, and the exact Darwin/Windows manifest and signature names. For the authorized Beta.23 channel, `internal-beta.yml` signs, notarizes, and staples macOS while publishing Windows as explicitly unsigned; any later production candidate must use `release-candidate.yml` and its stricter Authenticode boundary.
